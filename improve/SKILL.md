...
3. `.trail/learning.md` — the **compact learning surface** if present. A derived chronological extract of every `[!REALIZATION]` and `[!REVERSAL]` marker across the full trail, with date+slug context. Read this before `log.md` — it is what the loop has actually concluded across runs, and it surfaces learning that predates the most recent Retrospect run. Generated by `record.py learning --write`; if it looks stale relative to `log.md`'s most recent entries, regenerate it.
4. `.trail/log.md` — the full history. Read for the evidence behind anything in `learning.md` that needs its surrounding context, and for any specific prior decision the current run might contradict. With `learning.md` available, you should rarely need to read `log.md` end-to-end.
