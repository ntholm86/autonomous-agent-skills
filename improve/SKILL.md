...
3. `.trail/log.md` — the full history. Read for the evidence behind the orientation, and for any specific prior decision the current run might contradict. When the log is long, search for `[!REALIZATION]` and `[!REVERSAL]` markers — these are the settled conclusions and course-corrections from prior runs; reading them across the full history is more efficient than reading recent entries in full, and more likely to surface learning that predates the most recent Retrospect run.
