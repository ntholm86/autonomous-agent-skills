...
3. **Independent assessment.** Each evaluator scores the artifact fresh, without consulting prior scores. If evaluators anchor to the previous score, convergence measures social conformity, not quality. The de-anchoring rule is not optional — it is what makes convergence *observable* rather than *assumed*. In chat-based systems, switching to a new model inside the same conversation is **not** independent assessment — prior scores remain in context. A valid convergence evaluation must begin in a fresh conversation/session for that evaluator.
   ...
   **Independence extends to the measurement scheme, not just the score.** If the first evaluator derives what to measure and subsequent evaluators merely score against that scheme, independence is partial — subsequent scorers have accepted one family's framing of what *good* means. Scoring agreement under a shared scheme can reflect the scheme's blind spots rather than the artifact's quality. A genuinely independent evaluator re-derives the measurement scheme from the artifact before scoring, then compares against any inherited scheme. Convergence on re-derivation (the new family arrives at substantively the same measurements) validates the scheme. Divergence is a finding: the inherited scheme had a blind spot one family could not see. Record the re-derivation outcome — convergence or divergence — as part of the evaluation, regardless of the final score. Pre-agreed externally anchored rubrics (e.g., published standards) are exempt from re-derivation but not from the divergence-as-finding rule: if a new evaluator finds the rubric insufficient for this artifact, that is itself a finding.

**The minimum bar:** 3 consecutive runs, 3 distinct evaluator families, same score, zero artifact changes, and at least one re-derivation of the measurement scheme that converged with the inherited scheme. Below this, you have improvement trajectory. Above this, you have convergence. There is no middle ground.
